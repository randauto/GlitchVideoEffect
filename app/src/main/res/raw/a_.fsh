...
{
    return fract(sin(dot(seed * floor(iGlobalTime * 30.), vec2(127.1,311.7))) * 43758.5453123);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float r = rng2(uv);
    vec3 noise = (vec3(r, 1. - r, r / 2. + 0.5) * 1.0 - 2.0) * 0.05;

    float lineNoise = pow(rng2(blockS), 8.0) * pow(rng2(blockL), 3.0) - pow(rng(7.2341), 17.0) * 3.;
